...
def test_analyze_url(client):
    ...
    """
    with patch('scamscanner.api.v1.analyze.run_analysis_task') as mock_run_analysis_task:
        response = client.post("/analyze", json={"url": "http://example.com", "scan_depth": "deep", "use_domain_analyzer": True})
...
def test_analyze_code(client):
    ...
    job_id when the request is valid.
    """
    with patch('scamscanner.api.v1.analyze.run_code_audit_task') as mock_run_code_audit_task:
